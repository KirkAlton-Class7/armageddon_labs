...
resource "aws_lb_listener" "internal_app_public_alb" {
  load_balancer_arn = aws_lb.internal_app_public_alb.arn
  protocol          = "HTTP"
  port              = "80"
  ...
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.internal_app_asg_tg.arn
  }
}